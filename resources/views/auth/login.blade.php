@extends("layouts.layout_login")
@section('login')

<div class="container pt-5 pl-5">
    <div class="row justify-content-center">
        <div class="col-8 -md-8">
            <div class="card">
                <div class="card-header">{{ __('CONNEXION') }}</div>

                <div class="card-body">
                    <form method="POST" action="{{ route('login') }}">
                        @csrf

                        <div class="form-group row-10">
                            <label for="email" class="col-8-md-4 col-form-label text-md-right">{{ __('Email') }}</label>

                            <div class="col-8-md-6">
                                <input id="email" type="email" class="form-control @error('email') is-invalid @enderror" name="email" value="{{ old('email') }}" required autocomplete="email" autofocus>

                                @error('email')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                                @enderror
                            </div>
                        </div>

                        <div class="form-group row-12">
                            <label for="password" class="col-8-md-4 col-form-label text-md-right">{{ __('Mot de passe') }}</label>

                            <div class="col-8-md-6">
                                <input id="password" type="password" class="form-control @error('password') is-invalid @enderror" name="password" required autocomplete="current-password">

                                @error('password')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                                @enderror
                            </div>
                        </div>

                        <div class="form-group row-12">
                            <div class="col-md-6 offset-md-4">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="remember" id="remember" {{ old('remember') ? 'checked' : '' }}>

                                    <label class="form-check-label" for="remember">
                                        {{ __('Se souvenir de moi') }}
                                    </label>
                                </div>
                            </div>
                        </div>

                        <div class="form-group row-12 mb-0">
                            <div class="col-md-8 offset-md-4">
                                <button type="submit" class="btn btn-primary">
                                    {{ __('Connexion') }}
                                </button>

                                @if (Route::has('password.request'))
                                    <a class="btn btn-link" href="{{ route('password.request') }}">
                                        {{ __('mot de passe oublie?') }}
                                    </a>
                                @endif
                            </div>
                            <div class="form-group row-12 mb-0">
                            <div class="col-md-8 offset-md-4">
                                
                                <a class="nav-link" href="{{ route('register') }}">{{ __('Inscription') }}</a>
                                
                            <div class="form-group row-12 mb-0"> 
                            
                        
                        </div>
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </div>
</div>


@endsection